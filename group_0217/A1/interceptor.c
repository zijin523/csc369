...
/* An entry for each system  elscall in this "metadata" table */
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// If syscall is already being monitored for curret pid, then we pass tha argument.
	if (check_pid_monitored(sys, curr) == 1){
		// log_message.
		log_message(curr, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}else{
		// If syscall is being monitored for all pids, but current pid not in "ignore" list.
		if (table[sys].monitored == 2){
			// log_message.
			log_message(curr, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
	return table[sys].f(reg); // Just a placeholder, so it compiles with no warnings!
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	// check whether the syscall is valid or not;
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
		// check that the caller has the right permission;
		...
		switch(cmd){
			...
			case REQUEST_SYSCALL_INTERCEPT:
				...
				// stored orginal syscall into register
				table[syscall].f = sys_call_table[syscall];
				// set the syscall table as write permission
				set_addr_rw((unsigned long)sys_call_table);
				// write our descriptor in syscall table location
				sys_call_table[syscall] = interceptor;
				// set the syscall table as read only;
			...
			case REQUEST_SYSCALL_RELEASE:
				...
				// set the syscall table as write permission
				set_addr_rw((unsigned long)sys_call_table);
				// write orginal descriptor in syscall table location
	...
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
		// check whther the pid is valid or not for last two commands;
		// pid is negative or pid not exist except pid = 0;
		...
		// existing pid except pid = 0;
		if (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && pid != 0){
			return -EINVAL;
		}

		// check whether current syscall is intercepted or not;
		if(table[syscall].intercepted == 0){
			return -EINVAL;
		}
		...
		switch(cmd){
			case REQUEST_START_MONITORING:
				...
				// If pid not in de-monitored list which is ignore list and all pids are being
				// monitored;
				...
				// lock shared data;
				spin_lock(&my_table_lock);
				if (pid == 0){    // all pids should be monitored;
					// By possible way, change the my_list of current syscall into "ignore" list;
					// Or delet all de-monitored pids from ignore list because they are monitored from now;
					destroy_list(syscall);    // no pids are not mornitored by now;
					// allocate the space for "ignore" list;
					INIT_LIST_HEAD(&table[syscall].my_list);
					// update the status of attribute monitored;
					table[syscall].monitored = 2;
				}else if(table[syscall].monitored == 2){
					// Case: my_list is ignore list now;
					// Since we checked before, specific pid should
					// appear in ignore list;
					// Delete it from de-monitored list;
					if (del_pid_sysc(pid, syscall) != 0){
						// deleting failed, unlock avoid dead lock;
						spin_unlock(&my_table_lock);
						// display error;
						return -EINVAL;
					}
				}else{
					// Case: my_list is still monitored list;
					// We should add specific pid into monitored list;
					if (add_pid_sysc(pid, syscall) != 0){
						// adding failed, unlock avoid dead lock;
						spin_unlock(&my_table_lock);
						// display error;
						return -ENOMEM;
					}
					// adding successful, change the attribute status;
			...
			case REQUEST_STOP_MONITORING:
				...
				// If pid is in ignore list which is not being monitored;
				if (check_pid_monitored(syscall, pid) == 1 && table[syscall].monitored == 2){
					return -EINVAL;
				}
				...
				// lock shared data;
				spin_lock(&my_table_lock);
				// if we want to de-monitored all pids;
				if (pid == 0){
					// update status;
					table[syscall].monitored = 0;
					// Whatever the list is monitored or "ignored", we need to set it at initil
					// monitored list.
					destroy_list(syscall);
					// Initial the list, now it is monitored list;
					INIT_LIST_HEAD(&table[syscall].my_list);
				}else if (table[syscall].monitored == 2){
					// case: for monitored == 2, we just update the ignore list
					// which contains de-monitored pid;
					// adding de-monitored pid into ignore list;
					if (add_pid_sysc(pid, syscall) != 0){
						// adding failed, unlock avoid dead lock;
						spin_unlock(&my_table_lock);
						// display error;
					...
					// case: for monitored != 2, we delete it from monitored list
					// shows the pid is de-monitored;
					// lock shared data;
					
					// delete specific pid from monitored list;
					if (del_pid_sysc(pid, syscall) != 0){
						// deleting failed, unlock avoid dead lock;
						spin_unlock(&my_table_lock);
						// display error;
					...
					// Since delete function update the attribute, we don't
